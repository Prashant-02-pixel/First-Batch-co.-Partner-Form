...
export function PartnerOnboarding() {
  ...
  // Restore an in-progress partner record — step 1 is captured immediately.
  useEffect(() => {
    ...
    if (existing.detailedFormCompleted) setPhase("done");
    else if (existing.basicFormCompleted) setPhase("step2");
  }, []);
  ...
  const handleSaveAndContinue = () => {
    ...
    const base: PartnerRecord = record
      ? { ...record, basics, basicFormCompleted: true, detailedFormStarted: true, updatedAt: now }
      : {
    ...
    const next: PartnerRecord = isOther
      ? { ...base, detailedFormCompleted: true, status: "Profile Completed" }
      : base;
    setRecord(next);
    saveRecord(next);
    setPhase("saving");
    scrollToForm();
    window.setTimeout(() => setPhase(isOther ? "done" : "step2"), 1400);
  };
  ...
  const persistProfile = (updated: Profile) => {
    ...
    if (record) {
      const next: PartnerRecord = {
        ...
        profile: updated,
        detailedFormStarted: true,
        status: record.detailedFormCompleted ? record.status : "Profile In Progress",
  ...
  const showCommunityOptIn = track === "expert" || track === "consultant";

  const partnerLabel =
    PARTNER_TYPES.find((p) => p.id === basics.partnerType)?.label ?? "Partner";
    ...
    <div ref={containerRef} className="scroll-mt-24">
      <div className="mx-auto w-full max-w-3xl rounded-3xl border border-border bg-background p-6 shadow-[0_1px_2px_rgba(34,39,31,0.04)] sm:p-10">
        {phase !== "done" && phase !== "later" && (
          <ProgressHeader step={phase === "step1" || phase === "saving" ? 1 : 2} />
        )}
            ...
            <div className="mt-10 border-t border-border pt-8">
              {showCommunityOptIn && (
                <div className="step-fade mb-6">
                  <Checkbox
                    checked={basics.joinCommunity !== false}
                    onChange={(v) => setBasics((b) => ({ ...b, joinCommunity: v }))}
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                Your details are saved before moving to the next step.
              </p>
            </div>
          </div>
        )}
        ...
        {phase === "saving" && (
          <div className="step-fade mt-10 flex min-h-[22rem] flex-col items-center justify-center text-center">
            ...
            <p className="mt-5 max-w-md text-lg font-semibold">
              Thank you for sharing your details! We'll reach out to you soon. 😊
            </p>
          ...
          <div className="step-fade mt-8">
            <div className="mb-8 flex flex-wrap items-center gap-2 rounded-2xl bg-cream px-4 py-3 text-sm">
              ...
              <span className="text-muted-foreground">
                ...
                {basics.entity === "company" && basics.companyName
                  ? ` · ${basics.companyName}`
                  : ""}
              </span>
            </div>
            ...
            <StepTwo
              ...
              data={profile}
              set={(key, value) => persistProfile({ ...profile, [key]: value })}
            />
        ...
        {phase === "later" && (
          <div className="step-fade flex min-h-[20rem] flex-col items-center justify-center text-center">
            ...
            <h2 className="mt-5 text-2xl">No worries — we've saved your details.</h2>
            <p className="mt-2 max-w-md text-muted-foreground">
              Our team can follow up with you. You can pick up right where you left off whenever
              you're ready.
            </p>
            <button
              type="button"
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-lg font-semibold">
        {step === 1 ? "Partner Details" : "Partner Profile"}
      </p>
      <div className="mt-4 flex items-center gap-3">
        <Dot state={step === 1 ? "active" : "done"} label="1" />
        ...
        <Dot state={step === 2 ? "active" : "idle"} label="2" />
      </div>
    </div>
  );
}

function Dot({ state, label }: { state: "idle" | "active" | "done"; label: string }) {
  const cls =
    state === "active"
...
function SuccessState({ record }: { record: PartnerRecord | null }) {
  return (
    <div className="step-fade flex flex-col items-center py-6 text-center">
      ...
      <h2 className="mt-6 text-4xl sm:text-5xl">You're in!</h2>
      <p className="mt-3 text-lg font-semibold">Thanks for joining the First Batch network.</p>
      <p className="mt-3 max-w-md text-muted-foreground">
        We've received your profile. Our team will review your details and reach out if there's a
        relevant opportunity to collaborate.
      </p>
      {record && (
